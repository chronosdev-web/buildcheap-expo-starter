(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const m of o.addedNodes)m.tagName==="LINK"&&m.rel==="modulepreload"&&a(m)}).observe(document,{childList:!0,subtree:!0});function s(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(i){if(i.ep)return;i.ep=!0;const o=s(i);fetch(i.href,o)}})();class he{constructor(t,s){this.routes=t,this.app=s,this.currentRoute=null,window.addEventListener("hashchange",()=>this.resolve()),this.resolve()}resolve(){const t=window.location.hash.slice(1)||"/",s=t.split("?")[0],a=this.routes[s]||this.routes["/"];this.currentRoute!==t&&(this.currentRoute=t,this.app.innerHTML="",this.app.style.opacity="0",this.app.style.transform="translateY(8px)",a(this.app),requestAnimationFrame(()=>{this.app.style.transition="opacity 0.3s ease, transform 0.3s ease",this.app.style.opacity="1",this.app.style.transform="translateY(0)"}))}navigate(t){window.location.hash=t}}class fe{constructor(t={}){this._state={...t},this._listeners=new Map,this._globalListeners=new Set}get(t){return this._state[t]}getState(){return{...this._state}}set(t,s){const a=this._state[t];if(a===s)return;this._state[t]=s;const i=this._listeners.get(t);i&&i.forEach(o=>o(s,a,t)),this._globalListeners.forEach(o=>o(t,s,a))}update(t){for(const[s,a]of Object.entries(t))this.set(s,a)}on(t,s){return this._listeners.has(t)||this._listeners.set(t,new Set),this._listeners.get(t).add(s),()=>{const a=this._listeners.get(t);a&&(a.delete(s),a.size===0&&this._listeners.delete(t))}}onAny(t){return this._globalListeners.add(t),()=>this._globalListeners.delete(t)}reset(t={}){const s=this._state;this._state={...t};const a=new Set([...Object.keys(s),...Object.keys(t)]);for(const i of a)if(s[i]!==t[i]){const o=this._listeners.get(i);o&&o.forEach(m=>m(t[i],s[i],i))}}}const B=new fe({user:null,token:null,isAuthenticated:!1,currentPage:"/",sidebarOpen:!1,loading:!1,projects:[],builds:[],creditBalance:0,creditHistory:[],dashboardStats:null});function oe(e,t){B.update({user:e,token:t,isAuthenticated:!0})}function ve(){B.update({user:null,token:null,isAuthenticated:!1,projects:[],builds:[],creditBalance:0,dashboardStats:null})}const ne="/api",se={COST_PER_BUILD:.5};function xe(){return B.get("isAuthenticated")}function we(){return B.get("token")}async function u(e,t={}){const s={...t.headers};t.body instanceof FormData||(s["Content-Type"]="application/json");const a=await fetch(`${ne}${e}`,{...t,headers:s,credentials:"include"}),i=await a.text();let o;try{o=JSON.parse(i)}catch{throw console.error(`[apiFetch ERROR] URL: ${ne}${e}`),console.error(`[apiFetch ERROR] Status: ${a.status}`),console.error("[apiFetch ERROR] Response text:",i.substring(0,200)),new Error(`Failed parsing JSON for ${e}. Server returned HTML. Check dev tools console.`)}if(!a.ok)throw a.status===401&&(ve(),window.location.hash="#/login"),new Error(o.error||"Request failed");return o}const N={signup:async(e,t,s)=>{const a=await u("/auth/signup",{method:"POST",body:JSON.stringify({email:e,password:t,display_name:s})});return oe(a.user,a.token),a},login:async(e,t)=>{const s=await u("/auth/login",{method:"POST",body:JSON.stringify({email:e,password:t})});return oe(s.user,s.token),s},logout:async()=>{await u("/auth/logout",{method:"POST"}),ve(),window.location.hash="#/login"},me:async()=>{const e=await u(`/auth/me?t=${Date.now()}`);return B.update({user:e.user,token:e.token,isAuthenticated:!0}),e},update:async e=>{const t=await u("/auth/me",{method:"PUT",body:JSON.stringify(e)});return B.update({user:t.user}),t},uploadAvatar:async e=>{const t=await u("/auth/avatar",{method:"POST",body:JSON.stringify({avatar:e})});return B.update({user:t.user}),t},keys:{list:()=>u("/auth/keys"),create:(e,t)=>u("/auth/keys",{method:"POST",body:JSON.stringify({name:e,expiration_days:t})}),toggle:(e,t)=>u(`/auth/keys/${e}/toggle`,{method:"PUT",body:JSON.stringify({is_active:t})}),delete:e=>u(`/auth/keys/${e}`,{method:"DELETE"})}},Y={list:async()=>{const e=await u("/projects");return B.set("projects",e.projects),e},create:e=>u("/projects",{method:"POST",body:JSON.stringify(e)}),get:e=>u(`/projects/${e}`),update:(e,t)=>u(`/projects/${e}`,{method:"PUT",body:JSON.stringify(t)}),delete:e=>u(`/projects/${e}`,{method:"DELETE"}),upload:async(e,t)=>{const s=t.name.endsWith(".zip"),a=await fetch(`/api/projects/${e}/upload${s?"?format=zip":""}`,{method:"POST",headers:{"Content-Type":s?"application/zip":"application/gzip"},credentials:"include",body:t});if(!a.ok){const i=await a.json().catch(()=>({error:a.statusText}));throw new Error(i.error||"Upload failed")}return a.json()}},V={trigger:(e,t,s,a)=>u("/builds",{method:"POST",body:JSON.stringify({project_id:e,platform:t,commit_hash:s,commit_message:a})}),cancel:e=>u(`/builds/${e}/cancel`,{method:"POST"}),list:async(e=20,t=0)=>{const s=await u(`/builds?limit=${e}&offset=${t}&t=${Date.now()}`);return B.set("builds",s.builds),s},get:e=>u(`/builds/${e}`),getLog:e=>u(`/builds/${e}/log?t=${Date.now()}`),log:e=>u(`/builds/${e}/log?t=${Date.now()}`),stats:()=>u("/builds/stats")},ee={balance:async()=>{const e=await u("/credits");return B.set("creditBalance",e.balance),e},purchase:e=>u("/credits/purchase",{method:"POST",body:JSON.stringify({amount:e,success_url:`${window.location.origin}/#/billing?success=true`,cancel_url:`${window.location.origin}/#/billing?canceled=true`})}),history:async(e=50,t=0)=>{const s=await u(`/credits/history?limit=${e}&offset=${t}`);return B.set("creditHistory",s.transactions),s}},me={get:async()=>{const e=await u(`/dashboard?t=${Date.now()}`);return B.update({dashboardStats:e.stats,projects:e.projects,builds:e.recent_builds,creditBalance:e.user.credit_balance}),e}},J={apple:{get:()=>u("/credentials/apple"),save:e=>u("/credentials/apple",{method:"POST",body:JSON.stringify(e)}),delete:()=>u("/credentials/apple",{method:"DELETE"}),test:()=>u("/credentials/apple/test",{method:"POST"})},github:{save:e=>u("/credentials/github",{method:"POST",body:JSON.stringify({token:e})}),delete:()=>u("/credentials/github",{method:"DELETE"})}},te={list:()=>u("/webhooks"),add:e=>u("/webhooks",{method:"POST",body:JSON.stringify({url:e})}),delete:e=>u(`/webhooks/${e}`,{method:"DELETE"})},ae={list:e=>u(`/projects/${e}/secrets`),add:(e,t,s)=>u(`/projects/${e}/secrets`,{method:"POST",body:JSON.stringify({key_name:t,value:s})}),delete:(e,t)=>u(`/projects/${e}/secrets/${t}`,{method:"DELETE"})},W={list:async()=>u("/orgs"),create:async e=>u("/orgs",{method:"POST",body:JSON.stringify({name:e})}),get:async e=>u(`/orgs/${e}`),update:async(e,t)=>u(`/orgs/${e}`,{method:"PUT",body:JSON.stringify({name:t})}),delete:async e=>u(`/orgs/${e}`,{method:"DELETE"}),members:async e=>u(`/orgs/${e}/members`),invite:async(e,t,s)=>u(`/orgs/${e}/members`,{method:"POST",body:JSON.stringify({email:t,role:s})}),updateRole:async(e,t,s)=>u(`/orgs/${e}/members/${t}`,{method:"PUT",body:JSON.stringify({role:s})}),removeMember:async(e,t)=>u(`/orgs/${e}/members/${t}`,{method:"DELETE"})},ie={submit:async(e,t)=>u("/support",{method:"POST",body:JSON.stringify({title:e,description:t})}),listAll:async()=>u("/support/all"),resolve:async e=>u(`/support/${e}/resolve`,{method:"PUT"})};function Se(e,t){const s=B.get("token"),a=`${window.location.protocol==="https:"?"wss":"ws"}://${window.location.host}/ws`,i=new WebSocket(a);return i.onopen=()=>{s&&i.send(JSON.stringify({type:"auth",token:s})),i.send(JSON.stringify({type:"subscribe",buildId:e}))},i.onmessage=o=>{const m=JSON.parse(o.data);m.type==="log"&&t(m.line)},i}async function ye(){try{return await N.me(),!0}catch{return!1}}function ke(e){e.innerHTML=`
  ...
  `;let t=!1;const s=e.querySelector("#authForm"),a=e.querySelector("#nameGroup"),i=e.querySelector("#authTitle"),o=e.querySelector("#authSubtitle"),m=e.querySelector("#authSubmit"),v=e.querySelector("#authToggle"),b=e.querySelector("#authToggleText"),h=e.querySelector("#authError");v.addEventListener("click",()=>{t=!t,a.style.display=t?"flex":"none",i.textContent=t?"Create Account":"Sign In",o.textContent=t?"Start building for $0.50/build.":"Welcome back! Enter your credentials.",m.textContent=t?"Create Account":"Sign In",b.textContent=t?"Already have an account?":"Don't have an account?",v.textContent=t?"Sign In":"Sign Up",h.style.display="none"}),s.addEventListener("submit",async x=>{x.preventDefault(),h.style.display="none",m.disabled=!0,m.textContent=t?"Creating...":"Signing in...";try{const g=e.querySelector("#email").value,r=e.querySelector("#password").value;if(t){const p=e.querySelector("#displayName").value;if(!p)throw new Error("Display name is required");await N.signup(g,r,p)}else await N.login(g,r);window.location.hash="#/dashboard"}catch(g){h.textContent=g.message,h.style.display="block",m.disabled=!1,m.textContent=t?"Create Account":"Sign In"}})}function Ce(e){return{dashboard:"Dashboard",projects:"Projects",builds:"Builds",credentials:"Credentials",billing:"Billing",settings:"Settings",docs:"Documentation"}[e]||"BuildCheap"}function K(e,t){var o,m;const s=B.get("user")||{},a=document.createElement("div");a.className="app-layout",a.innerHTML=`
    <div class="bg-grid"></div>
  ...
  `;const i=a.querySelector("#pageContent");return typeof t=="string"?i.innerHTML=t:i.appendChild(t),(o=a.querySelector("#menuToggle"))==null||o.addEventListener("click",()=>{a.querySelector("#sidebar").classList.toggle("open")}),(m=a.querySelector("#logoutBtn"))==null||m.addEventListener("click",async()=>{if(confirm("Are you sure you want to log out?"))try{await N.logout()}catch(v){console.error("Logout failed:",v)}}),a}window.cancelBuild=async e=>{if(confirm("Are you sure you want to cancel this build? Your $0.50 credit will be refunded seamlessly."))try{await V.cancel(e),alert("Build cancelled successfully! Credit has been safely refunded to your account."),location.reload()}catch(t){alert("Failed to cancel build: "+t.message)}};function de(e){if(!e)return"—";const t=Math.floor(e/60),s=Math.round(e%60);return`${t}m ${s}s`}function Ee(e){if(!e)return"—";const t=(Date.now()-new Date(e).getTime())/1e3;return t<60?"just now":t<3600?`${Math.floor(t/60)} min ago`:t<86400?`${Math.floor(t/3600)} hr${Math.floor(t/3600)>1?"s":""} ago`:`${Math.floor(t/86400)} day${Math.floor(t/86400)>1?"s":""} ago`}function Be(e){return e==="success"?'<div class="build-status-icon success">✓</div>':e==="failed"?'<div class="build-status-icon error">✕</div>':e==="building"||e==="queued"?'<div class="build-status-icon building"><div class="spinner"></div></div>':'<div class="build-status-icon">—</div>'}function Te(e){return e==="ios"?'<span class="badge badge-info">iOS</span>':e==="android"?'<span class="badge badge-success">Android</span>':`<span class="badge">${e||"—"}</span>`}function Ae(e,t){const a=t.filter(i=>i.project_id===e.id)[0];return a?a.status==="building"||a.status==="queued"?'<span class="badge badge-warning" style="margin-left:auto;">Building</span>':a.status==="success"?'<span class="badge badge-success" style="margin-left:auto;">Active</span>':'<span class="badge badge-error" style="margin-left:auto;">Failed</span>':'<span class="badge" style="margin-left:auto;">No builds</span>'}function Le(){return`
    ...
    <div style="text-align:center;padding:var(--space-xl);color:var(--text-tertiary);">
      ...
      <div>${e}</div>
    </div>`}function Ie(e){const{user:t,stats:s,projects:a,recent_builds:i}=e,o=.5,m=2,v=(s.total_builds*o).toFixed(2),b=(s.total_builds*m).toFixed(2),h=(b-v).toFixed(2),x=b>0?Math.round(v/b*100):0,g=`
    ...
    <div class="card" style="padding:var(--space-lg);">
      <div style="text-align:center;margin-bottom:var(--space-md);">
        ...
        <div style="font-size:2rem;font-weight:900;color:var(--success);">$${h}</div>
  ...
  `}function ze(e){const t=Le(),s=K("dashboard",t);e.appendChild(s);const a=s.querySelector("#pageContent");me.get().then(i=>{a.innerHTML=Ie(i)}).catch(i=>{console.error("Dashboard load failed:",i),a.innerHTML=`<div style="padding:var(--space-xl);text-align:center;color:var(--text-tertiary);">
    ...
    <div style="display:flex;align-items:center;justify-content:center;min-height:300px;">
      <div class="spinner" style="width:32px;height:32px;"></div>
    </div>`}function le(e){if(!e)return"—";const t=(Date.now()-new Date(e).getTime())/1e3;return t<60?"just now":t<3600?`${Math.floor(t/60)} min ago`:t<86400?`${Math.floor(t/3600)} hr${Math.floor(t/3600)>1?"s":""} ago`:`${Math.floor(t/86400)} day${Math.floor(t/86400)>1?"s":""} ago`}function Pe(e){const t=_e(),s=K("projects",t);e.appendChild(s);const a=s.querySelector("#pageContent");function i(v){var x,g;if(!v||!v.projects||v.projects.length===0)return`
            <div class="page-title-bar">
                <h2>Projects</h2>
    ...
    `,h=v.projects.map((r,p)=>`
        ...
        <div class="card" style="padding:var(--space-md) var(--space-lg);margin-bottom:var(--space-lg);display:flex;align-items:center;justify-content:space-between;border-color:transparent;background:var(--surface-card);border:1px solid var(--border-medium);">
          ...
          <div style="width:250px;display:flex;align-items:center;gap:var(--space-sm);font-size:0.75rem;color:var(--text-tertiary);font-weight:600;">
            ...
            <div style="flex:1;height:6px;background:var(--border-medium);border-radius:3px;overflow:hidden;">
              <div style="height:100%;background:var(--primary);width:${Math.min(100,Math.max(0,Math.floor((((g=v.user)==null?void 0:g.credit_balance)||0)/se.COST_PER_BUILD)/100*100))}%;border-radius:3px;"></div>
            </div>
        ...
        `);const d=document.getElementById("customModalOverlay"),c=document.getElementById("repoUrlInput"),y=document.getElementById("uploadProjectName"),C=document.getElementById("cancelModalBtn"),S=document.getElementById("confirmModalBtn"),l=document.getElementById("createModalError"),k=document.getElementById("gitPathSection"),j=document.getElementById("uploadPathSection"),P=document.getElementById("tabGitBtn"),M=document.getElementById("tabUploadBtn"),D=document.getElementById("newProjectFileInput"),L=document.getElementById("newProjectDropZone"),H=document.getElementById("newProjectFileName");let T=null,I="git";c.focus(),L.addEventListener("click",()=>D.click()),D.addEventListener("change",()=>{D.files.length>0&&(T=D.files[0],H.innerText=T.name,L.style.borderColor="var(--primary)")}),L.addEventListener("dragover",_=>{_.preventDefault(),L.style.background="rgba(99,102,241,0.05)"}),L.addEventListener("dragleave",()=>{L.style.background="transparent"}),L.addEventListener("drop",_=>{_.preventDefault(),L.style.background="transparent",_.dataTransfer.files.length>0&&(T=_.dataTransfer.files[0],H.innerText=T.name,L.style.borderColor="var(--primary)")}),P.addEventListener("click",()=>{I="git",P.className="btn btn-primary",P.style.flex="1",M.className="btn btn-ghost",M.style.flex="1",k.style.display="block",j.style.display="none",S.innerText="Import Project"}),M.addEventListener("click",()=>{I="upload",M.className="btn btn-primary",M.style.flex="1",P.className="btn btn-ghost",P.style.flex="1",k.style.display="none",j.style.display="block",S.innerText="Create Project"});const z=()=>d.remove();C.addEventListener("click",z),S.addEventListener("click",async()=>{S.disabled=!0,l.style.display="none";const _=document.getElementById("createProjBundle").value.trim();try{if(I==="git"){const O=c.value.trim();if(!O){l.innerText="Repository URL is required",l.style.display="block",S.disabled=!1;return}S.innerText="Importing...";const q=O.replace(/\/$/,"").split("/"),w=q[q.length-1],n=q[q.length-2];await Y.create({name:w,slug:`${n||"user"}/${w}`,repo_url:O,bundle_id:_,platform:"ios"})}else{const O=y.value.trim();if(!O){l.innerText="Project name is required",l.style.display="block",S.disabled=!1;return}if(!T){l.innerText="Please select a .zip or .tar.gz source file",l.style.display="block",S.disabled=!1;return}S.innerText="Creating Project...";const q=O.toLowerCase().replace(/[^a-z0-9]/g,"-"),w=await Y.create({name:O,slug:q,bundle_id:_,platform:"ios"});S.innerText="Uploading Archive...",await Y.upload(w.project.id,T)}z(),o()}catch(O){l.innerText=O.message,l.style.display="block",S.disabled=!1,S.innerText=I==="git"?"Import Project":"Create Project"}})})}),a.querySelectorAll(".manage-secrets-btn").forEach(r=>{r.addEventListener("click",async p=>{const d=p.target.getAttribute("data-id"),y=`
  ...
  `;function m(d){if(!d||!d.builds||d.builds.length===0)return`
        ...
        `),!d&&o&&h(o)}).catch(c=>{const y=document.getElementById("buildsListCol");y&&(y.innerHTML=`<div style="padding:var(--space-xl);text-align:center;color:var(--error);">Failed to load builds: ${c.message}</div>`)})}let b=0;function h(d){b++;const c=b;i&&(i.close(),i=null);const y=document.getElementById("buildLogOutput");if(!y)return;y.innerHTML="Fetching logs...<br/>";let C=!1,S=[];V.log(d).then(l=>{c===b&&(l.log?y.textContent=l.log+`
`:y.textContent="",C=!0,S.forEach(k=>{const j=document.createElement("div");k.includes("✓")&&(j.style.color="var(--success)"),(k.includes("Error:")||k.includes("FAILED"))&&(j.style.color="var(--error)"),j.textContent=k,y.appendChild(j)}),S=[],y.scrollTop=y.scrollHeight)}).catch(()=>{c===b&&(y.textContent=`Failed to load historical logs.
`,C=!0)}),i=Se(d,l=>{if(c!==b)return;if(!C){S.push(l);return}const k=document.createElement("div");l.includes("✓")&&(k.style.color="var(--success)"),(l.includes("Error:")||l.includes("FAILED"))&&(k.style.color="var(--error)"),k.textContent=l,y.appendChild(k),y.scrollTop=y.scrollHeight}),i.addEventListener("message",l=>{JSON.parse(l.data).type==="build_complete"&&setTimeout(()=>v(!0),1e3)})}window.copyBuildLog=()=>{const d=document.getElementById("buildLogOutput");if(!d)return;const c=d.textContent||d.innerText;navigator.clipboard.writeText(c).then(()=>alert("Logs successfully copied to clipboard!")).catch(y=>alert("Browser copy failed: "+y))},window.selectBuild=d=>{o=d;const c=document.querySelectorAll(".build-item");c.forEach(C=>C.classList.remove("selected"));const y=Array.from(c).find(C=>{var S;return(S=C.getAttribute("onclick"))==null?void 0:S.includes(d)});y&&y.classList.add("selected"),h(d)};const x=`${window.location.protocol==="https:"?"wss":"ws"}://${window.location.host}/ws`;let g=null;function r(){g=new WebSocket(x),g.onopen=()=>{const d=we();d&&g.send(JSON.stringify({type:"auth",token:d}))},g.onmessage=d=>{try{const c=JSON.parse(d.data);(c.type==="build_complete"||c.type==="build_started")&&v(!0)}catch{}},g.onclose=()=>{document.body.contains(s)&&setTimeout(r,3e3)}}r();const p=setInterval(()=>{if(!document.body.contains(s)){clearInterval(p),i&&i.close(),g&&g.close();return}v(!0)},5e3);v()}function Oe(){return`
    <div style="display:flex;align-items:center;justify-content:center;min-height:300px;">
      <div class="spinner" style="width:32px;height:32px;"></div>
...
..." required style="font-family:monospace;font-size:12px;"></textarea>
        ...
        `}function He(e){const t=Oe(),s=K("credentials",t);e.appendChild(s);const a=s.querySelector("#pageContent");function i(){J.apple.get().then(b=>{B.get("user"),a.innerHTML=Ne(b)+m(),o(b.connected),v()}).catch(b=>{a.innerHTML=`<div style="padding:var(--space-xl);text-align:center;color:var(--error);">Failed to load credentials: ${b.message}</div>`})}function o(b){if(b){const h=a.querySelector("#deleteCredsBtn");h&&h.addEventListener("click",async g=>{if(g.preventDefault(),confirm("Are you sure you want to disconnect your Apple API Key? Your builds will fail until you reconnect!")){h.disabled=!0;try{await J.apple.delete(),i()}catch(r){alert("Failed to delete credentials: "+r.message),h.disabled=!1}}});const x=a.querySelector("#testCredsBtn");x&&x.addEventListener("click",async()=>{x.disabled=!0,x.innerText="Testing...";const g=a.querySelector("#testResult"),r=a.querySelector("#testBadge");g.innerHTML="";try{const p=await J.apple.test();r.className="badge badge-success",r.innerText="Valid",g.innerHTML=`<span style="color:var(--success);">✅ ${p.message}</span>`}catch(p){r.className="badge badge-error",r.innerText="Invalid",g.innerHTML=`<span style="color:var(--error);">❌ ${p.message}</span>`}x.disabled=!1,x.innerText="Test Connection"})}else{const h=a.querySelector("#appleCredsForm");h&&h.addEventListener("submit",async x=>{x.preventDefault();const g=a.querySelector("#saveCredsBtn"),r=a.querySelector("#credsError");g.disabled=!0,g.innerText="Saving...",r.style.display="none";try{await J.apple.save({issuer_id:a.querySelector("#issuerId").value,key_id:a.querySelector("#keyId").value,team_id:a.querySelector("#teamId").value,p8_key:a.querySelector("#p8Key").value}),i()}catch(p){r.innerText=p.message,r.style.display="block",g.disabled=!1,g.innerText="Save Credentials"}})}}function m(){return(B.get("user")||{}).has_github_token?`
    ...
    `:`
      ...
      `}function v(){const b=B.get("user")||{};if(b.has_github_token){const h=a.querySelector("#deleteGithubBtn");h&&h.addEventListener("click",async x=>{if(x.preventDefault(),confirm("Are you sure you want to disconnect your GitHub Token? Background clones of private repositories will immediately fail.")){h.disabled=!0;try{await J.github.delete(),await N.me(),i()}catch(g){alert("Failed to delete GitHub token: "+g.message),h.disabled=!1}}})}else{const h=a.querySelector("#githubCredsForm");h&&h.addEventListener("submit",async x=>{x.preventDefault();const g=a.querySelector("#saveGithubBtn"),r=a.querySelector("#githubCredsError"),p=a.querySelector("#githubToken");g.disabled=!0,g.innerText="Saving...",r.style.display="none";try{await J.github.save(p.value.trim()),B.set("user",{...b,has_github_token:!0}),i()}catch(d){r.innerText=d.message,r.style.display="block",g.disabled=!1,g.innerText="Save Access Token"}})}}i()}function Fe(e){const t=`
    <div class="page-title-bar">
      <h2>Billing</h2>
    </div>
  ...
  `,s=K("billing",t);e.appendChild(s);const a=e.querySelector("#creditBalance"),i=e.querySelector("#buildCount"),o=e.querySelector("#customAmount"),m=e.querySelector("#buildEstimate"),v=e.querySelector("#purchaseBtn"),b=e.querySelector("#purchaseError"),h=e.querySelector("#purchaseSuccess"),x=e.querySelector("#transactionHistory");o.addEventListener("input",()=>{const p=parseFloat(o.value)||0,d=Math.floor(p/.5);m.textContent=`= ${d} build${d!==1?"s":""}`}),e.querySelectorAll(".quick-amount").forEach(p=>{p.addEventListener("click",()=>{o.value=p.dataset.amount,o.dispatchEvent(new Event("input"))})}),v.addEventListener("click",async()=>{const p=parseFloat(o.value);if(b.style.display="none",h.style.display="none",!p||p<5){b.textContent="Minimum purchase is $5",b.style.display="block";return}v.disabled=!0,v.textContent="Processing...";try{const d=await ee.purchase(p);d.checkout_url&&(window.open(d.checkout_url,"_blank"),h.textContent="Redirecting to Stripe checkout... Complete payment in the new tab.",h.style.display="block")}catch(d){b.textContent=d.message,b.style.display="block"}v.disabled=!1,v.textContent="Purchase Credits"}),g(),r();async function g(){try{const p=await ee.balance();a.innerHTML=`<span class="text-gradient">$${p.balance.toFixed(2)}</span>`,i.textContent=`${p.available_builds} builds available`}catch{a.innerHTML='<span class="text-gradient">$0.00</span>',i.textContent="0 builds available"}}async function r(){try{const p=await ee.history();if(p.transactions.length===0){x.innerHTML=`
          <div class="card" style="padding:var(--space-lg);">
            <div class="empty-state">
  ...
  `,a=K("settings",s);e.appendChild(a);const i=a.querySelector("#apiKeysListContainer"),o=a.querySelector("#createKeyForm"),m=a.querySelector("#createKeyMsg"),v=a.querySelector("#keyExpirationSelect"),b=a.querySelector("#keyCustomExpirationInput");v&&b&&v.addEventListener("change",()=>{b.style.display=v.value==="custom"?"block":"none",v.value==="custom"&&b.focus()});let h=[];async function x(){if(i)try{if(h=(await N.keys.list()).keys||[],h.length===0){i.innerHTML='<div style="text-align:center;padding:var(--space-xl);color:var(--text-tertiary);">No API keys are currently active.</div>';return}i.innerHTML=h.map(n=>{let f="No expiration",E=!1;if(n.expires_at){const F=new Date(n.expires_at);E=F<new Date,f=E?"Expired":"Expires "+F.toLocaleDateString()}const $=E?"var(--error)":n.is_active?"var(--success)":"var(--text-tertiary)",R=E?"Expired":n.is_active?"Active":"Disabled";return`
        ...
        `}).join(""),a.querySelectorAll(".btn-copy-key").forEach(n=>{n.addEventListener("click",async f=>{const E=f.target.getAttribute("data-value");await navigator.clipboard.writeText(E),f.target.textContent="Copied!",setTimeout(()=>{f.target.textContent="Copy"},2e3)})}),a.querySelectorAll(".toggle-key-btn").forEach(n=>{n.addEventListener("click",async f=>{const E=f.target.getAttribute("data-id"),$=parseInt(f.target.getAttribute("data-active"),10);try{await N.keys.toggle(E,!$),x()}catch(R){alert(R.message)}})}),a.querySelectorAll(".delete-key-btn").forEach(n=>{n.addEventListener("click",async f=>{if(!confirm("Permanently delete this key? This action cannot be undone."))return;const E=f.target.getAttribute("data-id");try{await N.keys.delete(E),x()}catch($){alert($.message)}})})}catch(w){i.innerHTML=`<div style="color:var(--error);padding:var(--space-md);">${w.message}</div>`}}o&&o.addEventListener("submit",async w=>{w.preventDefault();const n=o.querySelector("#createKeyBtn"),f=o.querySelector("#newKeyName");m.style.display="none",n.disabled=!0;let E=v?v.value:30;if(E==="custom"&&(E=parseInt(b.value),isNaN(E)||E<1)){alert("Please enter a valid number of days for custom expiration"),n.disabled=!1;return}try{const $=await N.keys.create(f.value.trim(),E);f.value="",m.innerHTML='<span style="color:var(--success);">Key successfully generated! <strong>'+$.key.key_value+"</strong> Make sure to copy it now.</span>",m.style.display="block",x()}catch($){m.innerHTML='<span style="color:var(--error);">'+$.message+"</span>",m.style.display="block"}finally{n.disabled=!1}}),x();const g=a.querySelectorAll(".tab"),r=a.querySelector("#generalSection"),p=a.querySelector("#apiKeysSection"),d=a.querySelector("#teamSection"),c=a.querySelector("#webhooksSection"),y=a.querySelector("#notificationsSection");g.forEach(w=>{w.addEventListener("click",()=>{g.forEach(f=>f.classList.remove("active")),w.classList.add("active");const n=w.getAttribute("data-tab");[r,p,d,c,y].forEach(f=>{f&&(f.style.display="none")}),n==="general"?(r&&(r.style.display="block"),p&&(p.style.display="block")):n==="team"?(d&&(d.style.display="block"),k()):n==="webhooks"?c&&(c.style.display="block"):n==="notifications"&&y&&(y.style.display="block")})});const C=a.querySelector("#orgListContainer"),S=a.querySelector("#createOrgForm"),l=a.querySelector("#createOrgError");async function k(){try{const w=await W.list();if(!w.organizations||w.organizations.length===0){C.innerHTML='<div style="text-align:center;padding:var(--space-xl);color:var(--text-tertiary);">No organizations yet. Create one above!</div>';return}C.innerHTML=w.organizations.map(n=>`
          ...
          `).join(""),a.querySelectorAll(".view-members-btn").forEach(n=>{n.addEventListener("click",async()=>{const f=n.dataset.id,E=n.dataset.role,$=a.querySelector(`#members - ${f} `);if($.style.display==="block"){$.style.display="none";return}$.style.display="block",$.innerHTML='<div style="color:var(--text-tertiary);">Loading...</div>';try{const R=await W.members(f),F=["owner","admin"].includes(E);$.innerHTML=`
              ${F?`
                <div style="margin-bottom:var(--space-md);padding:var(--space-sm);background:var(--bg-secondary);border-radius:var(--radius-md);">
                  <form class="invite-form" data-org-id="${f}" style="display:flex;gap:var(--space-xs);align-items:center;flex-wrap:wrap;">
          ...
          ${R.members.map(A=>`
                  <div style="display:flex;align-items:center;justify-content:space-between;padding:var(--space-sm) 0;border-bottom:1px solid var(--border-subtle);">
                    ...
                    <div style="display:flex;align-items:center;gap:var(--space-xs);">
                      ...
                      ${F&&A.role!=="owner"?`<button class="btn btn-ghost btn-sm remove-member-btn" data-org-id="${f}" data-user-id="${A.user_id}" style="color:var(--error);font-size:0.75rem;">✕</button>`:""}
                    </div>
                  </div>
                `).join("")}
        </div>
        `;const G=$.querySelector(".invite-form");G&&G.addEventListener("submit",async A=>{A.preventDefault();const X=G.querySelector('input[type="email"]'),Z=G.querySelector("select"),Q=$.querySelector(".invite-error");Q.style.display="none";try{await W.invite(f,X.value.trim(),Z.value),X.value="",n.click(),n.click()}catch(be){Q.innerText=be.message,Q.style.display="block"}}),$.querySelectorAll(".remove-member-btn").forEach(A=>{A.addEventListener("click",async()=>{if(confirm("Remove this member?"))try{await W.removeMember(A.dataset.orgId,A.dataset.userId),n.click(),n.click()}catch(X){alert(X.message)}})})}catch(R){$.innerHTML=`<div style="color:var(--error);">${R.message}</div>`}})}),a.querySelectorAll(".delete-org-btn").forEach(n=>{n.addEventListener("click",async()=>{if(confirm("Delete this organization? All associated team access will be lost."))try{await W.delete(n.dataset.id),k()}catch(f){alert(f.message)}})})}catch(w){C.innerHTML=`<div style="color:var(--error);padding:var(--space-md);">${w.message}</div>`}}S&&S.addEventListener("submit",async w=>{w.preventDefault();const n=a.querySelector("#newOrgName");l.style.display="none";try{await W.create(n.value.trim()),n.value="",k()}catch(f){l.innerText=f.message,l.style.display="block"}});const j=a.querySelector("#webhooksListContainer"),P=a.querySelector("#webhookForm"),M=a.querySelector("#webhookUrlInput"),D=a.querySelector("#addWebhookBtn"),L=a.querySelector("#webhookErrorMsg");async function H(){try{const w=await te.list();if(!w.webhooks||w.webhooks.length===0){j.innerHTML='<div style="text-align:center;padding:var(--space-xl);color:var(--text-tertiary);">No webhooks configured.</div>';return}j.innerHTML=w.webhooks.map(n=>`
          ...
          `).join(""),a.querySelectorAll(".delete-webhook-btn").forEach(n=>{n.addEventListener("click",async f=>{if(!confirm("Remove this webhook?"))return;const E=f.target.getAttribute("data-id");n.disabled=!0;try{await te.delete(E),H()}catch($){alert("Failed to delete: "+$.message),n.disabled=!1}})})}catch(w){j.innerHTML=`<div style="color:var(--error);padding:var(--space-md);">${w.message}</div>`}}P&&(P.addEventListener("submit",async w=>{w.preventDefault(),D.disabled=!0,L.style.display="none";try{await te.add(M.value),M.value="",H()}catch(n){L.innerText=n.message,L.style.display="block"}finally{D.disabled=!1}}),H());const T=a.querySelector("#avatarUploadTrigger"),I=a.querySelector("#avatarFileInput"),z=a.querySelector("#avatarUploadMsg");T&&I&&(T.addEventListener("click",()=>I.click()),I.addEventListener("change",async w=>{const n=w.target.files[0];if(n){if(!n.type.startsWith("image/")){z.innerText="Please select an image file.",z.style.color="var(--error)",z.style.display="block";return}z.innerText="Uploading...",z.style.color="var(--text-tertiary)",z.style.display="block";try{const f=new Image,E=new FileReader;E.onload=()=>{f.onload=async()=>{const $=document.createElement("canvas");$.width=128,$.height=128;const R=$.getContext("2d"),F=Math.min(f.width,f.height),G=(f.width-F)/2,A=(f.height-F)/2;R.drawImage(f,G,A,F,F,0,0,128,128);const X=$.toDataURL("image/jpeg",.85);try{await N.uploadAvatar(X),z.innerText="Avatar updated!",z.style.color="var(--success)",setTimeout(()=>{e.innerHTML="",ge(e)},800)}catch(Z){z.innerText=Z.message||"Upload failed",z.style.color="var(--error)"}},f.src=E.result},E.readAsDataURL(n)}catch{z.innerText="Failed to process image.",z.style.color="var(--error)"}}}));const _=a.querySelector("#saveSettingsBtn"),O=a.querySelector("#displayNameInput"),q=a.querySelector("#settingsResultMsg");_&&O&&q&&_.addEventListener("click",async()=>{_.disabled=!0,_.innerText="Saving...";try{await N.update({display_name:O.value}),q.style.display="block",q.innerText="Saved successfully!",q.style.color="var(--success)",setTimeout(()=>{q.style.display="none"},3e3)}catch(w){q.style.display="block",q.innerText=w.message||"Failed to update",q.style.color="var(--error)"}finally{_.disabled=!1,_.innerText="Save Changes"}})}function Re(e){const s=K("cli",`
    <div class="page-title-bar">
      <h2>CLI Reference</h2>
